...
logger = logging.getLogger("hw3.q1") # homework3.q1
logger.setLevel(logging.DEBUG) #Logging messages which are less severe than logging.DEBUG will be ignored
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        N = len(sentence)
        for n in range(N):
            me = min(n+window_size+1,N)
            mb = max(0,n-window_size)
            startC = max(0,window_size-n)
            endC = max(0,n+window_size-N+1)
            temp = start*startC+sum(sentence[mb:me],[]) + end*endC
            
            windowed_data.append((temp,labels[n]))
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable('U',(self.config.hidden_size,Config.n_classes),
                        initializer = tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        loss_l = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits = pred)
